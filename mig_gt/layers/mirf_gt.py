...
class MMMLP(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        item_v_out_channels = (
            item_v_hidden_channels_list[-1]
            if len(item_v_hidden_channels_list) > 0
            else item_v_in_channels
        )
        item_t_out_channels = (
            item_t_hidden_channels_list[-1]
            if len(item_t_hidden_channels_list) > 0
            else item_t_in_channels
        )

        self.item_mlp = MyMLP(
            item_v_out_channels + item_t_out_channels,
...
class MIGGT(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.emb_mgdcf = (
            MGDCF(k_e, alpha, beta, 0.0, edge_drop_rate, 0.0) if k_e >= 0 else None
        )
        self.t_mgdcf = (
            MGDCF(k_t, alpha, beta, 0.0, edge_drop_rate, 0.0) if k_t >= 0 else None
        )
        self.v_mgdcf = (
            MGDCF(k_v, alpha, beta, 0.0, edge_drop_rate, 0.0) if k_v >= 0 else None
        )
        ...
        self.t_memory = nn.parameter.Parameter(
            torch.randn([1, num_clusters, item_t_in_channels])
            / np.sqrt(item_t_in_channels),
            requires_grad=False,
        )

        self.v_memory = nn.parameter.Parameter(
            torch.randn([1, num_clusters, item_v_in_channels])
            / np.sqrt(item_v_in_channels),
            requires_grad=False,
        )

        self.z_memory = nn.parameter.Parameter(
            torch.randn([1, num_clusters, user_in_channels])
            / np.sqrt(user_in_channels),
            requires_grad=False,
        )
    ...
    def forward(
        ...
        item_h = combined_h[num_users:] # [n_items, d]

        # combined_h.size(0): n vertex 
        memory_index = torch.randint(
            0, num_items, [combined_h.size(0), self.num_samples]
        ) # [n_vertex, n_samples]

        memory = item_h[memory_index] # [n_vertex, n_samples, d]

        # combined_h.unsqueeze(1): [n_vertex, 1, d]
        # memory: [n_vertex, n_samples, d]
        # concat output: [n_vertex, n_samples+1, d]
        memory = torch.concat([combined_h.unsqueeze(1), memory], dim=1)

        z_memory_h = self.z_transformer(memory, memory) # [n_vertex, n_samples+1, d]
        combined_h = z_memory_h[:, 0] # [n_vertex, d]
