...
class MGDCF(nn.Module):
    ...
    def __init__(
        self, k, alpha, beta, x_drop_rate, edge_drop_rate, z_drop_rate, *args, **kwargs
    ):
        """
        ...
        self.gammas = [
            torch.tensor(MGDCF.compute_gamma(alpha, beta, i)).float()
    ...
    def build_sorted_homo_graph(cls, user_item_edges, num_users=None, num_items=None):
        ...
        src = torch.concat(
            [user_index, homo_item_index, torch.arange(num_homo_nodes)], dim=0
        )
        dst = torch.concat(
            [homo_item_index, user_index, torch.arange(num_homo_nodes)], dim=0
        )
