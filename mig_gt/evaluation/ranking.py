...
def evaluate_mean_global_metrics(
    user_items_dict,
    ...
    max_mask_items_length = max(
        len(user_mask_items_dict[user]) for user in user_indices
    )

    _, user_rank_pred_items = v_search.search(
        embedded_users, k_list[-1] + max_mask_items_length
    )
    ...
    for user, pred_items in tqdm(zip(user_indices, user_rank_pred_items)):
        ...
        pred_items = [item for item in pred_items if item not in mask_items][
            : k_list[-1]
        ]
