...
def train_line(
    user_item_dict, num_users, num_items, nrl_pretrain_epochs=40, embedding_size=512
):
    ...
    user_embeddings = np.array(
        [word2vec_model.wv["u_{}".format(i)] for i in range(num_users)]
    )
    item_embeddings = np.array(
        [word2vec_model.wv["i_{}".format(i)] for i in range(num_items)]
    )
